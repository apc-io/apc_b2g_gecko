...
this.Sntp = function Sntp(dataAvailableCb, maxRetryCount, refreshPeriodInSecs,
  ...
  debug("Start request when initilize.");
  this._request();
}
...
Sntp.prototype = {
  ...
  _request: function _request() {
	...
	debug("We started the request=======");
      },

      onStopRequest: function onStopRequest(request, context, status) {
	debug("Request has stopped ====: " + status);
    ...
    let transport = socketTransportService
      .createTransport(["udp"],
                       ...
                       this._port,
                       null);
    debug("Let's check transport status : " + transport.isAlive());
