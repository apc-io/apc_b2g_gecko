this.EXPORTED_SYMBOLS = ["EthernetBackend", "EthernetServiceCommunicator",
                         ...
                         "EthernetSettingsCallBack", "Utils"];
...
this.EthernetBackend = {
  ...
  disconnectInterface: function EthernetUtil_disconnect(ifname) {
    ...
    if (iface.useDhcp) {
      ...
      EthernetStaticIPHelper.deconfigure(ifname, function() { debug("__ deconfigure in disconnect")});
  ...
  renewInterface: function EthernetUtil_renew(ifname) {
    ...
    if (EthernetSettings.useDhcp) {
      EthernetStaticIPHelper.deconfigure(ifname, function() { debug("====== after deconfigure ====")});
  ...
  updateIpConfig: function EthernetBackend_updateIpConfig(ifname) {
    ...
    var iface = EthernetIfUtils.getInterface(ifname);
    if (ifname.connected) {
      debug("Connection is present -> just reconfigure");
      EthernetStaticIPHelper.configure(ifname, NetUtilsCallbacks.onStaticIpConfigured);
    } else {
      debug("No connection => do connect  " + EthernetSettings.useDhcp);
      EthernetBackend.connectInterface(ifname);
    }
    EthernetBackend.callbackObj.onStaticConfigUpdated(Utils.createConnectionObject(StaticIPConfig));
  },
  ...
  ethernetStatsAvailable: function nsIEthernetStatsCallback_ethernetStatsAvailable(
    ...
    details.state = (details.up == true && details.cableConnected == true && Utils.validateIp(details.ip))
  ...
  _onInterfaceAdded: function(iface) {
    ...
    if (iface.name == kDefaultEthernetNetworkIface &&
      ...
      // alway do renew so we can make sure it is connected with the config we stored.
      EthernetBackend.renewInterface(iface.name);
    } else {
...
this.EthernetIfUtils = {
  ...
  createInterface: function EthernetIfUtil_createInterface(ifname, data) {
    let iface = {
      ...
      // prefixLength: data ? data.prefixLength : 0,
  ...
  updateInterface: function EthernetIfUtil_updateInterface(ifname, data) {
    ...
    this._updateProperty(iface, data, "ip", "ipaddr_str");
    ...
    if (ifname == kDefaultEthernetNetworkIface) { // should compare with currentIfName instead
      EthernetBackend.callbackObj.onConnectionUpdated(Utils.createConnectionObject(iface));
    }
...
this.EthernetStaticIPHelper = {
  ...
  configure: function EthernetStaticIPHelper_configure(ifname, callback) {
    debug("This is EthernetStaticIPHelper_configure");
    let workParams = {
      cmd: "ifc_configure",
      ifname: ifname,
  ...
  deconfigure: function EthernetStaticIPHelper_deconfigure(ifname, callback) {
    debug("This is deconfigure function for EthernetStaticIp");
    let workParams = {
      cmd: "ifc_reset_connections",
      ifname: ifname,
    };
...
this.Utils = {
  ...
  validateIp: function(ip) {
    var number = netHelpers.stringToIP(ip);
    return (number != 0 && number != null);
  },

  createConnectionObject: function(obj) {
    var mask = "";
    if (obj) {
      if ("prefixLength" in obj) {
        debug("well, we got the prefixLength = " + obj.prefixLength + " - " + obj["prefixLength"]);
        mask = netHelpers.ipToString(netHelpers.makeMask(obj.prefixLength));
      } else if ("mask_str" in obj) {
        debug("Ok, got the netmask via mask_str");
        mask = obj.mask_str;
      }
    ...
    var connection = {
      ip: obj ? this._getConnectionInfo(obj, "ip", "addrStr") : "",
      gateway: obj ? this._getConnectionInfo(obj, "gatewayStr", "gateway") : "",
      netmask: mask,
      dns1: obj ? this._getConnectionInfo(obj, "dns1Str", "dns1") : "",
      dns2: obj ? this._getConnectionInfo(obj, "dns2Str", "dns2") : ""
    };

    return connection;
  },

  _getConnectionInfo: function(obj, opt1, opt2) {
    var val = "";
    if (opt1 in obj) {
      val = obj[opt1];
    } else if (opt2 in obj) {
    ...
    if (val == "0.0.0.0") {
      val = ""; // the purpose of this is for displaying on the UX
    }
